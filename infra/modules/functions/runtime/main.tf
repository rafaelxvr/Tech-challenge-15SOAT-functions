locals {
  ...
  lambda_logs_statements = { for key, name in local.log_group_names : key => {
    Effect   = "Allow"
    Action   = ["logs:CreateLogStream", "logs:PutLogEvents"]
    Resource = ["arn:aws:logs:${var.aws_region}:${var.account_id}:log-group:${name}:log-stream:*"]
  } }
  vpc_statement = {
    Effect = "Allow"
    Action = [
  ...
  role_statements = {
    ...
    challenge = [
      ...
      },
      {
        ...
        Resource  = "arn:aws:ses:${var.aws_region}:${var.account_id}:identity/${var.ses_sender_email}"
...
resource "aws_iam_role_policy" "function" {
  ...
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = concat(
      [local.lambda_logs_statements[each.key], local.tracing_statement], each.key == "authorizer" ? [] : [local.vpc_statement],
      local.role_statements[each.key],
      [{ Sid = "ReadNewRelicIngestSecret", Effect = "Allow", Action = ["secretsmanager:GetSecretValue"], Resource = [var.newrelic_ingest_secret_arn] }]
    )
  })
}
...
resource "aws_lambda_function" "function" {
  ...
  dynamic "vpc_config" {
    for_each = each.key == "authorizer" ? [] : [true]
    content {
