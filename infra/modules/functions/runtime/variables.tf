...
variable "network" {
  ...
  validation {
    condition     = length(var.network.private_subnet_ids) >= 2 && alltrue([for subnet in var.network.private_subnet_ids : can(regex("^subnet-[a-f0-9]+$", subnet))]) && can(regex("^sg-[a-f0-9]+$", var.network.function_security_group_id))
