...
K8S owns the base HTTP API, VPC-link integrations, APP routes and stage. FUN owns the Lambda runtime, REQUEST authorizer, two public CPF routes and exact invoke permissions against the API ID/execution ARN supplied in `var.gateway`. `ownership_handoff_reviewed` defaults to false and blocks plans until an operator reviews the handoff inputs and exact ownership inventory. This source change neither edits K8S nor moves state.

The handoff must cover the four Lambda functions/roles/inline policies/log groups, source FIFO/DLQ, DynamoDB tables, mapping, publisher policy, and the single HTTP authorizer, its permission, two auth integrations/routes/invoke permissions. For an empty account, apply K8S first, pass its API ID/execution ARN to FUN, then export the canonical `gateway_handoff` object (`api_id`, `execution_arn`, `authorizer_id`, `environment`) back to K8S for protected APP routes. The exporter rejects mismatched API/ARN/environment fields, and K8S verifies the attestation against its owned API before creating protected routes. For an existing account, import existing identities into the owning root and remove former ownership without destroying live resources. The DLQ redrive-allow policy is explicit. Existing alarm/SNS resources, if deployed elsewhere, require the same single-owner review. Gateway API/stage, `/health`, business routes, and APP role attachment stay with K8S; FUN creates none of them. Review the SSE-KMS to SQS-managed encryption transition for any pre-existing queues and retain old-key consumer access until older encrypted messages have drained.
...
The verify workflow retains `mvnw verify` and adds offline pipeline contracts, Terraform fmt/validate/mock tests with read-only provider locks. The staging workflow requests OIDC only after its push/branch guard, uploads the source bundle to the fixed `releases/functions/staging/bundle.zip` CodeBuild source location with an immutable VersionId, keeps per-commit manifest and Terraform-input objects, then starts the exact private CodeBuild project and propagates its terminal status. The executor validates all digests, acquires the shared foundation lock, and applies only the staging root. Production remains disabled in both scripts; the archive buildspec remains inert because the private executor owns its inline bootstrap.

Before a staging run, configure the reviewed environment variables/secrets and the private CodeBuild inline bootstrap/IAM for the FUN-owned API Gateway bindings, SQS redrive policies, DynamoDB TTL/PITR, native alarms/SNS and pinned Lambda layers. The source workflow does not create those external controls. No production cloud readiness is claimed.

`contracts/outputs-allowlist.json` exports the canonical `gatewayHandoff`, authorizer ID, `customerPublicKeys` (reviewed public RSA JWKs indexed by kid), four function ARNs, source queue URL/ARN, DLQ ARN, publisher policy ARN, artifact digest and native alarm topic ARN. The public JWK input must include the current kid and match the independently initialized signing/trust material; Terraform does not decrypt that material to prove the match. `scripts/export-outputs.ps1` reads an already obtained Terraform output JSON file, validates the handoff's API/ARN/environment binding, rejects missing/sensitive allowlisted fields and drops unknown values. It does not read AWS or state itself and never publishes secret values.
...
I7 now has a staging-only launch path with offline branch/artifact/window/lock validation; production activation remains disabled. See [the exact partial prerequisites](i7-pipeline-contracts.md).
