...
public final class FunctionFactory {
    ...
    public static FunctionFactory fromEnvironment() {
        try {
            Clock clock = Clock.systemUTC();
            ConnectionProvider provider = new ConnectionProvider(new ConnectionProvider.Config(required("DB_HOST"), integer("DB_PORT"),
                    required("DB_NAME"), required("DB_USER"), required("DB_PASSWORD"), Path.of(required("DB_CA_PATH")).toAbsolutePath()));
            ...
            DesafioStore store = new DynamoDesafioStore(dynamo, required("CHALLENGE_TABLE"), hasher, clock);
            EnviarCodigo email = new SesEnviarCodigo(ses, required("OTP_SENDER"));
            PrivateKey privateKey = privateKey(required("CUSTOMER_PRIVATE_KEY_B64"));
            PublicKey publicKey = publicKey(required("CUSTOMER_PUBLIC_KEY_B64"));
            String issuer = required("JWT_ISSUER"), audience = required("JWT_AUDIENCE"), kid = required("CUSTOMER_KEY_ID");
            ...
            byte[] secret = Base64.getDecoder().decode(required("STAFF_HMAC_SECRET_B64"));
            StaffTokenVerifier staff = new StaffTokenVerifier(Map.of(required("STAFF_KEY_ID"), new SecretKeySpec(secret, "HmacSHA256")), issuer, audience, clock);
            StructuredLog.coldStart("auth-functions");
            return new FunctionFactory(criar, verificar, new Authorizer(customer, staff, new RoutePolicy()));
        } catch (RuntimeException exception) {
            throw new IllegalStateException("Function configuration unavailable");
        }
    }

    private static String required(String name) { String value = System.getenv(name); if (value == null || value.isBlank()) throw new IllegalArgumentException(); return value; }
    private static int integer(String name) { try { return Integer.parseInt(required(name)); } catch (NumberFormatException e) { throw new IllegalArgumentException(); } }
